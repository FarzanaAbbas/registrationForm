...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        //hash the password
        //         What is Salt?
        // A salt is a random piece of data added to a password before it’s hashed. It serves as an additional input to the hashing function. The primary purpose of salt is to make each hashed password unique, even if two users have the same password.

        // Prevents Identical Hashes: Without salt, if two users have the same password (e.g., "password123"), their hashed passwords would also be the same. This could allow attackers to guess passwords more easily or use precomputed hashes to crack them.
